...
use crate::{
    ...
    wake::{is_actionable_wake, is_closed_wake, is_untagged_actionable_wake, mentions_step},
};
...
impl WakeScanner {
    ...
    pub(in crate::wait) fn select(
        ...
        if let Some((offset, line)) =
            select_wake_with_offset(&lines[start..], self.index, closed_wake_match(target))
        {
...
fn select_state_attributed_wake(
    ...
    let Some(attributed) = target_attributed_step(target, index)? else {
        return Ok(None);
    };
...
fn closed_wake_match(target: &WaitTarget) -> ClosedWakeMatch {
    match target {
...
fn target_attributed_step(target: &WaitTarget, index: usize) -> Result<Option<AttributedStep>> {
    let WaitTarget::Run { run_dir, .. } = target else {
        return Ok(None);
    };
...
fn read_optional_run_state(run_dir: &Utf8Path) -> Result<Option<RunState>> {
    let path = crate::store::state_path(run_dir);
