...
When your task is a review, a security audit is part of it, whether or not the brief asks. Enumerate threat classes against the change rather than only checking the brief's list: secret and PII leakage, authz and authn bypass, injection, and — most often missed — resource exhaustion and amplification. For anything that accepts or forwards attacker-influenced input, bound both directions: request size, response size, and any parameter that multiplies work (array lengths, fan-out counts, encodings, recursion depth). Treat an unbounded read of an upstream or user-supplied body as a finding by default. Ask what one request can cost in memory, time, money, and blast radius on the deployment topology the change actually ships to.
