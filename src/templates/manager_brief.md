...
- **Never scope a reviewer away from security.** Reviewers audit for security as a standing duty, so a delta-scoped re-review still covers the delta's threat classes. Your risk-focus list is additive, never a substitute: an omitted threat class reads as out of scope. For a change that is itself a security boundary — anything internet-facing, authenticating, or forwarding untrusted input — commission a dedicated adversarial pass ("attack this") alongside the correctness review, rather than assuming the code reviewer generalizes to it.
