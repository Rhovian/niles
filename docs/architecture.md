...
Bare `niles` is tmux-only. If `$TMUX` is unset and stdin/stdout are TTYs, it starts an attached `niles` tmux session and re-runs the original argv inside it; non-TTY launches fail with instructions to start or attach tmux interactively. Its launch prelude creates `.niles/worker/` and interactively ensures `.niles/manifest.yaml` exists before the foreground manager starts.
