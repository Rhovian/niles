...
Adding `--run` generates the manifest and prepares a run through the same path as `niles run`. The supervisor advances prepared runs one step at a time with `niles step` for interactive agent windows or `niles exec-step` for captured command/agent steps.
...
`niles resume` reloads the original task file for a persisted run, verifies that the step shape still matches the saved run state, keeps completed steps, resets the first incomplete step and later steps, and prints the next command for the supervisor to run. This is intentionally narrower than full checkpointing, but it is enough to recover from failed validation or interrupted agent steps during early product trials.
...
The supervisor-driven step runner is the only execution mode.
